      ...
      <template v-for="(barItem, barIndex) in barList" :key="barIndex">
        <el-sub-menu :index="barItem.path" v-if="barItem.children">
          <template v-slot:title>
            <i class="mg-r-8" :class="barItem.icon"></i>
          ...
          <el-menu-item
            ...
            >{{ subBarItem.text }}</el-menu-item
          >
        </el-sub-menu>
        <el-menu-item
          v-if="!barItem.children"
          :index="barItem.path"
          @click="activeHandle(barItem)"
        >
          <i class="mg-r-8" :class="barItem.icon"></i>
          <template v-slot:title>
            <span class="text">{{ barItem.text }}</span>
...
export default defineComponent({
  setup() {
    ...
    function initBar() {
      ...
      state.defaultPath =
        path.includes("/article/list") ||
        path.includes("/resource/list") ||
